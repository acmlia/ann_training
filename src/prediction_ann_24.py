...
#        dataset = dataset.iloc[NaN_pixels]
        ...
        y_pred = pd.DataFrame(y_pred, index=dataset_index,columns=['y_pred'])

        pos = np.where((y_pred['y_pred']>0))
        y_pred = y_pred.iloc[pos]
        df_final_index = y_pred.index.values
    
        y_true = y_true.iloc[pos]
        ...
        val_y_pred_mean, val_y_true_mean, val_mae, val_rmse, val_std, val_fseperc, val_fse, val_corr, val_num_pixels, val_bias, val_armse = skills.metrics(y_true, y_pred)
        ...
        # ------------------------------------------------------------------------------
        # ------------------------------------------------------------------------------
        df_final = df_orig.iloc[df_final_index]
        df_final['y_true'] = y_true
